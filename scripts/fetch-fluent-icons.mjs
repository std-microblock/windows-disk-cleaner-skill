...
const pin =
  process.argv[2] || JSON.parse(await fetchText("https://api.github.com/repos/" + repository + "/commits/main")).sha;
const icons = {
  drive: "Hard Drive",
  folder: "Folder",
...
const dir = path.join(root, "ui", "icons", "fluent");
...
for (const [local, name] of Object.entries(icons)) {
  const source =
    ...
    "_20_regular.svg";
  try {
    const text = await fetchText("https://raw.githubusercontent.com/" + repository + "/" + pin + "/" + source);
    await fs.writeFile(path.join(dir, local + ".svg"), text, "utf8");
    console.log(local);
  } catch (e) {
...
await fs.writeFile(
  ...
  await fetchText("https://raw.githubusercontent.com/" + repository + "/" + pin + "/LICENSE"),
  "utf8",
);
await fs.writeFile(
  path.join(dir, "NOTICE.md"),
  "Selected, unmodified 20px regular Fluent System Icons.\nCopyright Microsoft Corporation. Licensed under MIT (see LICENSE).\nUpstream: https://github.com/" +
    repository +
    "\nRevision: " +
    pin +
